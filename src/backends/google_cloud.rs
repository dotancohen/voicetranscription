...
impl GoogleCloudBackend {
    ...
    pub fn new(config: BackendConfig) -> Result<Self> {
        let access_token = config
            .api_key
            .ok_or_else(|| TranscriptionError::InvalidConfig("Google Cloud access token required".into()))?;

        let project_id = config
            .options
            .get("project_id")
            .cloned()
            .ok_or_else(|| TranscriptionError::InvalidConfig("Google Cloud project_id required".into()))?;
    ...
    fn parse_response(
                        ...
                        if let Some(words) = &alt.words {
                            ...
                            for word in words {
                                if let (Some(start), Some(end)) = (&word.start_offset, &word.end_offset) {
                                    ...
                                    .with_confidence(alt.confidence.unwrap_or(0.0) as f64);
        ...
        TranscriptionResult::with_segments(full_text.trim().to_string(), segments)
    }
