...
impl AssemblyAIBackend {
    ...
    pub fn new(config: BackendConfig) -> Result<Self> {
        let api_key = config
            .api_key
            .ok_or_else(|| TranscriptionError::Configuration("AssemblyAI API key required".into()))?;
    ...
    async fn upload_file(&self, audio_path: &Path) -> Result<String> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| TranscriptionError::Backend(format!("Upload failed: {}", e)))?;
        ...
        let upload_response: UploadResponse = response
            .json()
            .await
            .map_err(|e| TranscriptionError::Backend(format!("Failed to parse upload response: {}", e)))?;
    ...
    async fn start_transcription(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| TranscriptionError::Backend(format!("Failed to start transcription: {}", e)))?;
    ...
    async fn poll_transcription(&self, transcript_id: &str) -> Result<TranscriptResponse> {
        ...
        for attempt in 0..MAX_POLL_ATTEMPTS {
            let response = self
                ...
                .send()
                .await
                .map_err(|e| TranscriptionError::Backend(format!("Poll request failed: {}", e)))?;
            ...
            let transcript: TranscriptResponse = response
                .json()
                .await
                .map_err(|e| TranscriptionError::Backend(format!("Failed to parse poll response: {}", e)))?;
    ...
    fn format_result(&self, transcript: TranscriptResponse) -> TranscriptionResult {
        ...
        let segments = transcript.words.map(|words| {
            words
                .iter()
                .map(|w| Segment {
        ...
        TranscriptionResult {
            ...
            duration: transcript.audio_duration.map(|d| d as f64),
        }
    }
}
